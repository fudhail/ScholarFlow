...
from app.agents.nodes import (
    ...
    rag_response_node,
    # Phase 2 — Pre-guardrail preprocessing
    continuity_detector,
    topic_queue_manager,
    interrupt_handler,
    ...
    deep_rag_pipeline_node,
    llm_necessity_gate,
    deterministic_handler,
    # Phase 4 — Planner reviewer
...
from app.agents.routing import (
    ...
    route_after_llm_gate,
    route_after_planner_reviewer,
)
...
def _recommended_asset_types_for_section(title: str) -> list[str]:
    lowered = (title or "").strip().lower()
    if lowered in {"methodology", "methods", "materials and methods"}:
        return ["data"]
    if lowered in {"results", "discussion"}:
        return ["image", "data"]
    return []


def _parse_outline_sections(outline_text: str, selected_paper_ids: list[str]) -> list[dict]:
    sections: list[dict] = []
    ...
    for raw_line in (outline_text or "").splitlines():
        ...
        if line.startswith("## "):
            ...
            current = {
                ...
                "description": "",
                "status": "pending",
                "relevant_paper_ids": list(selected_paper_ids),
                "recommended_asset_types": _recommended_asset_types_for_section(title),
            }
            continue
        if current:
...
AVAILABLE DATA: {len(lab_asset_ids) + len(research_asset_ids)} project assets
...
"""
    ...
    sections_plan = _parse_outline_sections(outline_text, selected_paper_ids)
...
def route_after_intent(state: ResearchState) -> Literal["search_subgraph", "drafting_subgraph", "lab_analyst", "writer", "grounded_chat"]:
    ...
    if intent == "SEARCH":
        ...
        return "lab_analyst"
    elif intent == "GROUNDED_CHAT":
        logger.info("-> Routing to grounded_chat")
...
def create_research_graph():
    ...
    # ── Phase 2: Pre-guardrail preprocessing ──
    graph.add_node("interrupt_handler", interrupt_handler)
    graph.add_node("continuity_detector", continuity_detector)
    graph.add_node("topic_queue_manager", topic_queue_manager)

    # ── Phase 3: Guardrail pipeline ──
    graph.add_node("intent_classifier_fast", intent_classifier_fast)
    graph.add_node("intent_guardrail", intent_guardrail_node)
    graph.add_node("deep_rag_pipeline", deep_rag_pipeline_node)
    graph.add_node("llm_necessity_gate", llm_necessity_gate)
    graph.add_node("deterministic_handler", deterministic_handler)
    graph.add_node("guardrail_response", lambda state: {
        "workflow_state": "blocked",
        "logs": [{"step": "guardrail_response", "source": "GuardrailBlock",
                  "message": "🚫 Request blocked by guardrail.", "status": "completed"}]
    })
    ...
    # ===== ENTRY POINT =====
    # All queries enter through the guardrail pipeline first.
    # interrupt_handler → continuity_detector → topic_queue_manager
    # → intent_classifier_fast → intent_guardrail → llm_necessity_gate
    # → [deterministic_handler | router → downstream agents]
    graph.set_entry_point("interrupt_handler")


    # ===== GUARDRAIL PIPELINE (runs before supervisor/memory/router) =====
    #
    # Full pre-guardrail flow:
    # interrupt_handler → continuity_detector → topic_queue_manager
    #   → intent_classifier_fast → intent_guardrail → llm_necessity_gate
    #   → deterministic_handler | planner → planner_reviewer → downstream

    graph.add_conditional_edges(
        "interrupt_handler",
        route_after_interrupt,
        {"supervisor": "continuity_detector", "END": END}
    )
    graph.add_edge("continuity_detector", "topic_queue_manager")
    graph.add_edge("topic_queue_manager",  "intent_classifier_fast")
    graph.add_edge("intent_classifier_fast", "intent_guardrail")
    graph.add_conditional_edges(
        "intent_guardrail",
        route_after_guardrail,
        {
            "router": "supervisor",          # ALLOW → normal flow via supervisor
            "clarifier": "clarifier",         # CLARIFY → ask user
            "guardrail_response": "guardrail_response",
            "deep_rag_pipeline": "deep_rag_pipeline"
        }
    )
    graph.add_edge("deep_rag_pipeline", "search")
    graph.add_edge("guardrail_response", END)

    # ===== SUPERVISOR / MEMORY / ROUTER (original orchestrated flow) =====

    # Supervisor → Memory → LLM gate (instead of going straight to router)
    graph.add_edge("supervisor", "memory")
    graph.add_edge("memory",     "llm_necessity_gate")
    graph.add_conditional_edges(
        "llm_necessity_gate",
        route_after_llm_gate,
        {"planner": "router", "deterministic_handler": "deterministic_handler"}
    )
    graph.add_edge("deterministic_handler", END)
    ...
    graph.add_node("planner_reviewer", planner_reviewer_node)
    graph.add_edge("planner", "planner_reviewer")
    graph.add_conditional_edges(
        "planner_reviewer",
        route_after_planner_reviewer,
        {
            "planner":   "planner",       # REVISE — loop back
            "writer":    "writer",         # APPROVED, needs writing
            "search":    "search",         # APPROVED, needs research
            "synthesis": "synthesis",      # APPROVED, needs synthesis
        }
    )
